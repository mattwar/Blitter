...
//
//     dotnet run CustomShaderRimLight.cs
//
...
//
// `N` is the surface normal in world space; `V` is the direction from the surface point back toward the camera.
// Faces pointing straight at the camera get rim ~= 0; faces grazing the silhouette get rim ~= 1. 
// Adding a colored `rim * RimColor` term on top of a basic Lambertian shade is
...
var torus  = Meshes.Torus(new Color(180, 60, 40), majorRadius: 0.9f, minorRadius: 0.32f, majorSegments: 64, minorSegments: 96);
...
await window.RunAsync(rd =>
{
    ...
    var sphereModel = Matrix4x4.CreateRotationY(t * 0.4f).Translate(-1.4f, 0f, 0f);
...
public struct RimArgs : IUniformArgs<RimArgs>
{
    ...
    // allows the renderer to inject the camera's view-projection matrix into the args
    // without boxing the args struct.
