...
public class Motion3DTests
{
    ...
    {
        var sprite = new Sprite3D { Velocity = new Vector3(2f, 0f, 0f) };
        var motion = new Motion3D();

        motion.Apply(sprite, Ctx(0.5));
    ...
    public void IntegratesAngularVelocityIntoOrientation()
    {
        ...
        motion.Apply(sprite, Ctx(1.0)); // one second => half turn
    ...
    {
        var sprite = new Sprite3D { Position = new Vector3(1f, 2f, 3f) };
        var motion = new Motion3D();

        motion.Apply(sprite, Ctx(0.1));
