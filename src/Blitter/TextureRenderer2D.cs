...
/// <summary>
/// Common base for 2D renderers that render into a texture surface
/// </summary>
internal abstract class TextureRenderer2D : Renderer2D, IDisposable
{
    ...
    private protected TextureRenderer2D(nint rendererId)
    {
    ...
    public override bool DrawImage(Texture2D image, Rect source, Rect destination)
    {
        if (IsDisposed)
            return false;
        TextureRegion2D.Unwrap(ref image, ref source);
