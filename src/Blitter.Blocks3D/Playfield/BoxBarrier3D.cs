...
public class BoxBarrier3D : Barrier3D
{
    ...
    // Cached for sphere-vs-OBB: transforms a world vector into the box's
    // local frame.
    ...
    public BoxBarrier3D(Vector3 center, Vector3 halfExtents, Quaternion rotation)
    {
        ...
        Rotation = rotation;
        _inverseRotation = Quaternion.Conjugate(rotation);
    }
    ...
    public override bool Intersects(BoundingSphere sphere)
    {
        ...
        // Rotate the sphere's offset into the box's local frame, then
        // clamp to the box; standard sphere-vs-OBB closest-point test.
        var local = Vector3.Transform(sphere.Center - Center, _inverseRotation);
        var clamped = Vector3.Clamp(local, -HalfExtents, HalfExtents);
        var d = local - clamped;
        return d.LengthSquared() <= sphere.Radius * sphere.Radius;
    }
}
