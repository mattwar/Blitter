...
/// <summary>
/// Bridges a <see cref="SparseVoxelWorld"/> into the chunked playfield layer. 
/// Each generated <see cref="Chunk3D"/> owns one <see cref="VoxelChunkBarrier3D"/> covering the matching voxel chunk; 
/// the playfield drives streaming via its <see cref="ChunkedPlayField3D.MinChunk"/> / <see cref="ChunkedPlayField3D.MaxChunk"/> range.
/// </summary>
public class VoxelChunkSource3D : GeneratedChunkSource3D
{
    ...
    {
        // Drop the underlying voxel array too so unbounded walking doesn't leak memory. 
        // A revisit will regenerate the chunk deterministically from the same generator inputs.
